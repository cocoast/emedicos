    ...
    <table id="garantiastable" class="table table-striped table-hover mt-4" style="width:100%">
        <thead>
            <tr>
                ...
                <th scope="col">Mp Disponibles</th>                
        ...
        @foreach($garantias as $garantia)
            <tr>
                <td>     <!-- Trigger the modal with a button -->
                    <button type="button" data-path="{{route('garantia.edit', $garantia->id) }}" class="btn btn-warning btn-sm openBtn">
                    {{$garantia->id}}</button>
                    @can('equipo.destroy')
                        <form action="{{route('garantia.destroy',$garantia->id)}}" method="POST">
                    @csrf
                    @method('DELETE')
         ...
         <!-- Trigger the modal with a button -->
                ...
                <td>{{ $garantia->Equipo->Proveedor->nombre }}</td>   
                
                <td>{{ $garantia->mp_disponible }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
